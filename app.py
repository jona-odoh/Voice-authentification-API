...
def enroll():
    ...
    for recording1 in base_records:
        ...
        w = wave.open(os.path.join(
            app.config['UPLOAD_FOLDER'], filename1), 'rb')
        recordings.append([w.getparams(), w.readframes(w.getnframes())])
        w.close()
        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], filename1))

    outputFile = wave.open(os.path.join(
        app.config['UPLOAD_FOLDER'], filename0), 'wb')
    ...
    gmm, gmm_model_path = add_user(username, os.path.join(
        app.config['UPLOAD_FOLDER'], filename0))
...
def authentificate():
    ...
    if model is not None:
        ...
        identity, score = reconize_with_model(os.path.join(
            app.config['UPLOAD_FOLDER'], filen), "./temp/models/" + username + ".gmm", username)
        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        ...
        identity, score = recognize(os.path.join(
            app.config['UPLOAD_FOLDER'], filen), username)
        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], filen))
        if identity == username:
            output['score'] = round(score*100, 2)
